...
export function WorkProgressCard({ dayTasks, todaySessions, projects, selectedDate }: WorkProgressCardProps) {
  // 予定時間の計算（分単位）
  ...
  const projectStats = projects.map(project => {
    ...
    const taskStats = projectTasks.map(task => {
      ...
      const taskActualSeconds = taskSessions.reduce((sum, session) => sum + session.duration_seconds, 0)
      ...
      <CardContent className="py-4 space-y-4">
        <div className="text-center">
          <div className="text-lg font-medium font-mono">
            {isTodaySelected ? todayTotalFormatted : formatTime(actualMinutes)} / {formatTime(plannedMinutes)}
          </div>
        </div>
                    ...
                    {tasks.map(({ task, actualMinutes, plannedMinutes }) => (
                      ...
                      >
                        <span className={cn(
                          ...
                          <span className={cn(
                            task.status === 'completed' ? "text-muted-foreground/60" : "text-foreground"
                          )}>
                            {task.name}
                          </span>
                        </span>
