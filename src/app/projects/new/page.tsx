...
import { Calendar, Clock, AlertTriangle, Plus, CalendarDays } from 'lucide-react'
import { Checkbox } from '@/components/ui/checkbox'
import { toast } from 'sonner'
import { useProjectCreationStore, Task } from '@/stores/project-creation-store'
import { useProjects } from '@/hooks/use-projects'
import { useBigTasks } from '@/hooks/use-big-tasks'
import { format, differenceInDays } from 'date-fns'
import { FlowTaskTable } from '@/components/project/flow-task-table'
import { RecurringTaskTable } from '@/components/project/recurring-task-table'
...
export default function ProjectCreatePage() {
  ...
  useEffect(() => {
    calculateTotalWeeks()
    calculateWeeklyHours()
    // クライアント側でのみ初期タスクを作成（Hydrationエラー対策）
    initializeTasks()
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.error(
            'BigTask作成中にエラーが発生しました。ロールバックを実行します。',
            taskError
          )
        }
        ...
        <div className="space-y-6">
          ...
          <Card className="rounded-lg border">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4">
                <ProjectColorPicker value={projectColor} onChange={setProjectColor} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-lg border">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <RecurringTaskTable
                  tasks={tasks.filter(t => t.task_type === 'recurring')}
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">（一度だけ実行するタスク）</span>
                </div>
                <FlowTaskTable
                  tasks={tasks.filter(t => t.task_type !== 'recurring')}
                  totalTaskHours={tasks.filter(t => t.task_type !== 'recurring').reduce((sum, t) => sum + t.estimatedHours, 0)}
                  projectCategories={projectCategories}
                  onAddTask={() => {
                    const newTask: Task = {
                      id: `task-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                    ...
                    setTimeout(() => {
                      const lastTask = tasks[tasks.length - 1]
                      if (lastTask) {
                        updateTask(lastTask.id, { task_type: 'flow' })
                      }
                    }, 0)
                  }}
                  onUpdateTask={updateTask}
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {tasks.filter(t => t.task_type !== 'recurring').reduce((sum, t) => sum + t.estimatedHours, 0).toFixed(1)}時間
                    </span>
                  </div>
                  <div className="flex justify-between items-center mt-1">
                    <span className="text-sm text-muted-foreground">定期タスク</span>
                    <span className="text-sm text-muted-foreground">
                      {tasks.filter(t => t.task_type === 'recurring').reduce((sum, t) => {
                        if (!t.recurrence) return sum
                        const frequency = t.recurrence.frequency
                        const hoursPerOccurrence = t.recurrence.hours_per_occurrence || 0
