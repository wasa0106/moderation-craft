...
export const getNextColor = (existingCategories: CategoryColor[]): string => {
  ...
  const availableColors = DEFAULT_CATEGORY_COLORS.filter(color => !usedColors.includes(color))
  ...
  const baseColor =
    DEFAULT_CATEGORY_COLORS[existingCategories.length % DEFAULT_CATEGORY_COLORS.length]
