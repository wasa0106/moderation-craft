...
export function useScheduleMemo(userId: string, currentWeek: Date) {
  ...
  const weekStartDate = startOfWeek(currentWeek, { weekStartsOn: 1 }).toISOString().split('T')[0]
    ...
    queryFn: async () => {
      try {
        ...
        if (
          ...
          (error.message.includes('UpgradeError') || error.message.includes('DatabaseClosedError'))
        ) {
  ...
  const saveScheduleMemoMutation = useMutation({
    mutationFn: async (content: string) => {
      ...
      await syncService.addToSyncQueue(
        'schedule_memo',
        memo.id,
        scheduleMemoQuery.data ? 'update' : 'create',
        memo
      )
      return memo
    },
  ...
  // Get recent memos
  const getRecentMemos = useCallback(
    async (limit?: number) => {
      try {
